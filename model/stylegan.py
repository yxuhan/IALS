'''
PyTorch implementation of the StyleGAN Generator, by Piotr Bialecki and Thomas Viehmann.
We simply paste the source code from https://github.com/lernapparat/lernapparat/blob/master/style_gan/pytorch_style_gan.ipynb to here. 
'''
...
class G_synthesis(nn.Module):
    def __init__(self,
        ...
        torgbs = []
        blocks = []
        last_channels, channels = None, None
...
def get_stylegan(pretrain_path, device):
    g_all = nn.Sequential(OrderedDict([
        ('g_mapping', G_mapping()),
        ('g_synthesis', G_synthesis())
    ])).to(device)
    g_all.load_state_dict(torch.load(pretrain_path))
    g_mapping, g_style = g_all[0], g_all[1]
    return g_mapping, g_style
